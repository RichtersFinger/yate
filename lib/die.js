...
class Die {
	...
	redraw() {
		...
		this.ctx.drawImage(this.image, 0.04 * this.image.width, 0.04 * this.image.width, 0.92 * this.image.width, 0.92 * this.image.width);//0, 0);//
	}
	setdisplayposition() {
	...
	roll() {
		this.value = Math.floor(1 + this.maxvalue*random());//Math.random());
		iosocket.emit('diceroll', this.value, this.maxvalue);
		this.applyimage();
	}
}
