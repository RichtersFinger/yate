...
class TokenContainer {
	...
	loadproperties(sometoken) {
		for (var i = 0; i < relevantdata_tokenframe.length; i++) {
			this[relevantdata_tokenframe[i]] = sometoken[relevantdata_tokenframe[i]];
		}
		if (this.descriptionisprepared) {
			...
			var editfunction = (function (tokenreference){
							return function(){	
								if (tokenreference.editmode) {
									...
									tokenreference.desctext = tokenreference.desctextparagraphedit.value;
									...
									tokenreference.desctextparagraphedit.value = tokenreference.desctext;
