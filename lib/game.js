	...
	**************************************************/
	$.contextMenu({
		...
		build: function($trigger, e) {
			...
			if (player === 0) {
				/*thismenu.items["createtestcard"] = {"name": "New Test Deck", "callback": function(itemKey, opt){
												var newdeck = new LinkedList();
												var somedeckid = idcounter++;
												for (var i = 0; i < 5; i++) {
													newdeck.addToTail(new Card(somedeckid, i, newdeck, gamediv, InitialMouseDownX+i*5, InitialMouseDownY+i*5, 672/3, 936/3));
												}
												decks.addToTail(newdeck);
											}
							   };
				thismenu.items["createtestcard2"] = {"name": "New Test Deck 2", "callback": function(itemKey, opt){
												var newdeck = new LinkedList();
												var somedeckid = idcounter++;
												for (var i = 0; i < 5; i++) {
													newdeck.addToTail(new Card(somedeckid, i, newdeck, gamediv, InitialMouseDownX+i*5, InitialMouseDownY+i*5, 672/3, 936/3, "img/_server_/placeholdercard.jpg", "img/_server_/placeholdercardb.jpg"));
												}
												decks.addToTail(newdeck);
											}
							   };*/
				thismenu.items["createcanvas"] = {"name": "New Canvas", "callback": function(itemKey, opt){
												var somethingx = new CanvasFrameContainer(1, gamediv, InitialMouseDownX, InitialMouseDownY, 400, 400, "");
												//var canvas = new fabric.Canvas('c');
												//canvas.isDrawingMode = true;
												...
												canvas.renderAll();
												*/
												//var logthis = function(){console.log(canvas.upperCanvasEl);canvas.upperCanvasEl.renderAll();};
												//canvas.on('mouse:up', logthis);
											}
							   };
			...
			if (topframe) {
				if (topframe.what === ContainerTypes.Card ) {
					...
					if (topframe.filenameback === logopath && !topframe.faceup && topframe.viewingrights.includes(player)) {
						var currentcardlist;
												...
												document.getElementById("fileinputdialog").onchange = function(){
													...
													asyncLoadLocalFile(this.files[0], function(contents){
															...
															if (words[1] == "1.2") {
																...
																counter = 4;
																var gameoptions0 = words[2].split(',');
																for (var i = 0; i < gameoptions0.length; i++) {
																	iosocket.emit('gameoptions_add', gameoptions0[i]);
																}
															} else {
																console.log("Unknown file version.");
																return;
															}
															console.log('Loaded file has version', words[1]);
															do {
																...
																switch (parseInt(words[counter++])) {
																  ...
																  case ContainerTypes.TokenContainer:
																    ...
																    break;
																  case ContainerTypes.Card:
																	var tempcard = {};
																	for (var i = 0; i < relevantdata_card.length; i++) {
																		if (relevantdata_card[i] === "streamposition" || 
																		     relevantdata_card[i] === "faceup") {
																			if (words[counter].toLowerCase() === "true") 
																				words[counter] = true;
																			else if (words[counter].toLowerCase() === "false") 
																				words[counter] = false;
																		}
																		tempcard[relevantdata_card[i]] = words[counter];
																		counter++;
																	}
																	var currentdeck = decks.head;
																	var correctdeck = null;
																	...
																	newcard.loadproperties(tempcard);
																	
																	correctdeck.addToTail(newcard);
																	
																	// parse owner-array
																	var tmpowners = tempcard.owner.split(",");
																	for (var j = 0; j < tmpowners.length; j++)
																		tmpowners[j] = parseInt(tmpowners[j]);
																	newcard.owner = tmpowners;
																	// parse viewingrights-array
																	var tmpviewingrights = tempcard.viewingrights.split(",");
																	for (var j = 0; j < tmpviewingrights.length; j++)
																		tmpviewingrights[j] = parseInt(tmpviewingrights[j]);
																	newcard.viewingrights = tmpviewingrights;
																	
																	var currentdate = new Date();
																	newcard.timestamp = currentdate.getTime() - clienttimereference + servertimereference;
																    break;
																}
																counter++;
