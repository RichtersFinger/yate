...
var relevantdata_canvasframe = ["id", "owner", "streamposition", "streamcontent", "fixposition", "transparentbackground", "timestamp", "x", "y", "width", "height", "scale", "content", "zIndex"];
var relevantdata_canvasframe_types = {"id": "number", "owner":"numberarray", "streamposition":"boolean", "streamcontent":"boolean", "fixposition":"boolean", "transparentbackground": "boolean", "timestamp": "number", "x": "number", "y": "number", "width": "number", "height": "number", "scale": "number", "content":"string", "zIndex": "number"};
		...
		if (publicdieframes[someid]) {
			var timer = setInterval(function () {
				...
				if (!publicdieframes[someid].justrolled) {
					clearInterval(timer);
					publicdieframes[someid].image.removeEventListener('load', publicdieframes[someid].redrawfunction);
...
function playanimations() {
		...
		if (unpushedlist[i]) {
			if (unpushedlist[i].thisdiv) {
				...
				if (unpushedlist[i].what === ContainerTypes.TokenContainer) {
					...
					if (unpushedlist[i].what === ContainerTypes.PublicDieFrame || unpushedlist[i].what === ContainerTypes.Card) {
	...
	if (Object.keys(animatedPointer).length > 0 || unpushedlist.length > 0) {
