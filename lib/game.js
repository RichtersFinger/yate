	...
	**************************************************/
	$.contextMenu({
		...
		build: function($trigger, e) {
			...
			if (e.target === document.getElementById("notesdiv") || e.target === notesdiv_textdisplaydiv ) {
				if (!editingnotes) {
			...
			if (player === 0 && !topframe) {
				...
				thismenu.items.gameoptionsubmenu.items["addoption"] = {"name": "Add Option", "callback": function(itemKey, opt){
												var newoption = prompt("Current settings: " + gameoptions.join(' ') + "; supported notes d<deckid>dragtotop, d<deckid>randomizeangles, ...", "");
												if (newoption) {
