...
var relevantdata_publicdieframe = ["id", "owner", "timestamp", "x", "y", "value", "maxvalue", "filenamebase", "filenametype", "fixposition", "setable", "size", "scale", "zIndex"];
var relevantdata_publicdieframe_types = {"id":"number", "owner":"numberarray", "timestamp":"number", "x":"number", "y":"number", "value":"number", "maxvalue":"number", "filenamebase":"string", "filenametype":"string", "fixposition":"boolean", "setable":"boolean", "size":"number", "scale":"number", "zIndex":"number"};
var imageframes = {};
var tokenframes = {};
var dieframes = {};
var publicdieframes = {};
...
$(function(){
	...
	// select german tts voice if possible
	...
	iosocket.on('updatepublicdieframe', ( someframe ) => {
		...
		publicdieframes[someframe.id].value = someframe.value;
		publicdieframes[someframe.id].applyimage();
		adjustzCounter();
	});
		...
		build: function($trigger, e) {
			...
			if (player === 0 && topframe) {
				...
				if (topframe.what === ContainerTypes.FrameContainer || topframe.what === ContainerTypes.TokenContainer || topframe.what === ContainerTypes.Marker || topframe.what === ContainerTypes.FrameLabel || topframe.what === ContainerTypes.CanvasFrame || topframe.what === ContainerTypes.LotteryFrame || topframe.what === ContainerTypes.PublicDieFrame) {
					...
					if (topframe.what === ContainerTypes.PublicDieFrame) {
						if (topframe.setable)
							thismenu.items.framedetails.items["setabletoggle"] = {"name": "Setable: On", "callback": function(itemKey, opt){
																	if (topframe.setable) {
																		topframe.setable = false;
																		opt.items.framedetails.items[itemKey].$node[0].innerHTML = "<span>Setable: Off</span>";
																	} else {
																		topframe.setable = true;
																		opt.items.framedetails.items[itemKey].$node[0].innerHTML = "<span>Setable: On</span>";
																	}
																	return false;}};
						else
							thismenu.items.framedetails.items["setabletoggle"] = {"name": "Setable: Off", "callback": function(itemKey, opt){
																	if (topframe.setable) {
																		topframe.setable = false;
																		opt.items.framedetails.items[itemKey].$node[0].innerHTML = "<span>Setable: Off</span>";
																	} else {
