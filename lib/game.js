...
var relevantdata_lotteryframe = ["id", "owner", "viewingrights", "timestamp", "x", "y", "currentindex", "options", "streamposition", "fixposition", "selectatrandom", "playsound", "publicresult", "scale", "zIndex"];
var relevantdata_lotteryframe_types = {"id":"number", "owner":"numberarray", "viewingrights":"numberarray", "timestamp":"number", "x":"number", "y":"number", "currentindex":"number", "options":"stringarray", "streamposition":"boolean", "fixposition":"boolean", "selectatrandom":"boolean", "playsound":"boolean", "publicresult":"boolean", "scale":"number", "zIndex":"number"};
var imageframes = {};
var tokenframes = {};
var dieframes = {};
var decks = {};
			...
			if (player === 0) {
				...
				if (topframe) {
					if (topframe.what === ContainerTypes.FrameContainer || topframe.what === ContainerTypes.CanvasFrame || topframe.what === ContainerTypes.TokenContainer || topframe.what === ContainerTypes.LotteryFrame) {
						...
						console.log(topframe.what, topframe.zIndex);
						if (e.deltaY < 0 && topframe.zIndex < 2) return false;
						// change layer
						// look for other id
						var correctframe = null;
						for (var marker in topframe.parentframe.marker) {
							console.log(topframe.parentframe.marker[marker]);
		...
		build: function($trigger, e) {
			...
			if (player === 0) {
				thismenu.items["createlottery"] = {"name": "New Lottery", "callback": function(itemKey, opt){
												...
												var newid = idcounter++;
												lotteryframes[newid] = new LotteryFrame(newid, gamediv, InitialMouseDownX, InitialMouseDownY, ["asd", "asd2", "asd3", "longer content"]);
												adjustzCounter();
											}
							   };
			...
			if (player === 0 && topframe) {
				...
				if (topframe.what === ContainerTypes.FrameContainer || topframe.what === ContainerTypes.TokenContainer || topframe.what === ContainerTypes.Marker || topframe.what === ContainerTypes.FrameLabel || topframe.what === ContainerTypes.CanvasFrame || topframe.what === ContainerTypes.LotteryFrame) {
					if (topframe.what === ContainerTypes.TokenContainer)
					...
					if (topframe.what === ContainerTypes.FrameContainer || topframe.what === ContainerTypes.TokenContainer || topframe.what === ContainerTypes.CanvasFrame || topframe.what === ContainerTypes.LotteryFrame) {
					...
					if (topframe.what === ContainerTypes.FrameContainer || topframe.what === ContainerTypes.TokenContainer || topframe.what === ContainerTypes.CanvasFrame || topframe.what === ContainerTypes.LotteryFrame) {
						...
						if (topframe.what === ContainerTypes.LotteryFrame) {
							...
							if (topframe.publicresult)
								thismenu.items.framedetails.items["publicresulttoggle"] = {"name": "Public Result: On", "callback": function(itemKey, opt){
																		if (topframe.publicresult) {
																			topframe.publicresult = false;
																			opt.items.framedetails.items[itemKey].$node[0].innerHTML = "<span>Public Result: Off</span>";
																		} else {
																			topframe.publicresult = true;
																			opt.items.framedetails.items[itemKey].$node[0].innerHTML = "<span>Public Result: On</span>"; 
																		}
																		return false;}};
							else
								thismenu.items.framedetails.items["publicresulttoggle"] = {"name": "Public Result: Off", "callback": function(itemKey, opt){
																		if (topframe.publicresult) {
																			topframe.publicresult = false;
																			opt.items.framedetails.items[itemKey].$node[0].innerHTML = "<span>Public Result: Off</span>";
																		} else {
					...
					if (topframe.what === ContainerTypes.FrameContainer || topframe.what === ContainerTypes.CanvasFrame || topframe.what === ContainerTypes.LotteryFrame) {
			...
			if (player === 0 && !topframe) {
				...
				thismenu.items.gameoptionsubmenu.items["addoption"] = {"name": "Add Option", "callback": function(itemKey, opt){
												var newoption = prompt("Current settings: " + gameoptions.join(' ') + ";\n supported d<deckid>dragtotop, d<deckid>randomizeangles, ...", "");
												if (newoption) {
