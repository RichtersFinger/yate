...
$(function(){
	...
	iosocket.on('updatecard', ( somecard ) => {
		...
		if (correctcard) {
			...
			correctdeck.addToTail(new Card(somecard.deckid, somecard.cardid, gamediv, somecard.x, somecard.y, somecard.width, somecard.height, somecard.filenamefront, somecard.filenameback));
				...
				if (topframe) {
					...
					if (e.shiftKey) {
						...
						if (e.detail === 1) {
							...
							if (multiselected) {
								if (topframe.what === ContainerTypes.Card) {
									if (iscardinstack(multiselectlist, draggedframe.deckid, draggedframe.cardid)) {
										var current = multiselectlist.head;
										for (var i = 0; i < multiselectlist.length; i++) {
	...
	**************************************************/
	$.contextMenu({
		...
		build: function($trigger, e) {
			...
			if (player === 0) {
				thismenu.items["createtestcard"] = {"name": "New Test Card", "callback": function(itemKey, opt){
												if (decks.length === 0) {
													decks.addToTail(new LinkedList());
													decks.addToTail(new LinkedList());
												}
												decks.head.value.addToTail(new Card(1, idcounter++, gamediv, InitialMouseDownX, InitialMouseDownY, 672/3, 936/3));
											}
							   };
				thismenu.items["createtestcard2"] = {"name": "New Test Card 2", "callback": function(itemKey, opt){
												if (decks.length === 0) {
													decks.addToTail(new LinkedList());
													decks.addToTail(new LinkedList());
												}
												decks.tail.value.addToTail(new Card(2, idcounter++, gamediv, InitialMouseDownX, InitialMouseDownY, 672/3, 936/3, "img/_server_/placeholdercard.jpg", "img/_server_/placeholdercardb.jpg"));
											}
							   };
			...
			if (topframe) {
				if (topframe.what === ContainerTypes.Die) {
					...
					if (topframe.faceup || topframe.filenameback !== logopath) { // note: oder falls Rechte vorhanden, um verdeckte Karte anzusehen
						thismenu.items["sep1"] = "---------";
						thismenu.items["viewcard"] = {"name": "View Card", "callback": function(itemKey, opt){
														var currentimage;
														if (topframe.faceup) {
															currentimage = topframe.imagefront;
														} else {
															currentimage = topframe.imageback;
														}
														var borderwidth = 3;
														...
														if (topframe.faceup) {
															detailimagedivimg.src = topframe.filenamefront;
														} else {
															detailimagedivimg.src = topframe.filenameback;
														}
			...
			if (player === 0 && topframe) {
				...
				if (topframe.what === ContainerTypes.FrameContainer || topframe.what === ContainerTypes.TokenContainer || topframe.what === ContainerTypes.Marker || topframe.what === ContainerTypes.FrameLabel) {
					if (topframe.what === ContainerTypes.TokenContainer)
					...
					thismenu.items["cardstacksubmenu"] = { "name": "Stack", "items": {} };
					thismenu.items["carddecksubmenu"] = { "name": "Deck", "items": {} };
					
					
					// implement this using var currentcardlist = {topframe, multiselectlist, decks.?};
					thismenu.items.cardsinglesubmenu.items["pushthis"] = {"name": "Push This", "callback": function(itemKey, opt){
													// send to server for broadcast
													var currentdate = new Date();
													topframe.timestamp = currentdate.getTime() - clienttimereference + servertimereference;
													iosocket.emit('pushcard', topframe);
												}
								   };								   
					if (topframe.what === ContainerTypes.Card) {
						// build owner+view submenu+callback
						thismenu.items.cardsinglesubmenu.items["movingsubmenu"] = { "name": "Set Moving Rights", "items": {} };
						for (var i = 0; i < playernames.length; i++) {
							if (topframe.owner.includes(i))
								thismenu.items.cardsinglesubmenu.items.movingsubmenu.items["movingsubmenukey"+i] = {"name": playernames[i], "callback": function(itemKey, opt){
																		var thisplayer =  parseInt(itemKey.replace("movingsubmenukey", ""));
																		if (topframe.owner.includes(thisplayer)) {
																			topframe.owner.splice(topframe.owner.indexOf(thisplayer), 1);
																			opt.items.cardsinglesubmenu.items.movingsubmenu.items[itemKey].$node[0].classList.add("disabledtext"); 
																		} else {
																			topframe.owner.push(thisplayer);
																			opt.items.cardsinglesubmenu.items.movingsubmenu.items[itemKey].$node[0].classList.remove("disabledtext"); 
																		}
																		return false;}};//
							else
								thismenu.items.cardsinglesubmenu.items.movingsubmenu.items["movingsubmenukey"+i] = {"name": playernames[i], "className": "disabledtext", "callback": function(itemKey, opt){
																		var thisplayer =  parseInt(itemKey.replace("movingsubmenukey", ""));
																		if (topframe.owner.includes(thisplayer)) {
																			topframe.owner.splice(topframe.owner.indexOf(thisplayer), 1);
																			opt.items.cardsinglesubmenu.items.movingsubmenu.items[itemKey].$node[0].classList.add("disabledtext"); 
						...
						thismenu.items.cardsinglesubmenu.items["viewingsubmenu"] = { "name": "Set Viewing Rights", "items": {} };
						for (var i = 0; i < playernames.length; i++) {
							if (topframe.viewingrights.includes(i))
								thismenu.items.cardsinglesubmenu.items.viewingsubmenu.items["viewingsubmenukey"+i] = {"name": playernames[i], "callback": function(itemKey, opt){
																		...
																		if (topframe.viewingrights.includes(thisplayer)) {
																			...
																			opt.items.cardsinglesubmenu.items.viewingsubmenu.items[itemKey].$node[0].classList.add("disabledtext"); 
																		} else {
																			topframe.viewingrights.push(thisplayer);
																			opt.items.cardsinglesubmenu.items.viewingsubmenu.items[itemKey].$node[0].classList.remove("disabledtext"); 
																		}
																		return false;}};//
							else
								thismenu.items.cardsinglesubmenu.items.viewingsubmenu.items["viewingsubmenukey"+i] = {"name": playernames[i], "className": "disabledtext", "callback": function(itemKey, opt){
																		var thisplayer =  parseInt(itemKey.replace("viewingsubmenukey", ""));
																		if (topframe.viewingrights.includes(thisplayer)) {
																			topframe.viewingrights.splice(topframe.viewingrights.indexOf(thisplayer), 1);
																			opt.items.cardsinglesubmenu.items.viewingsubmenu.items[itemKey].$node[0].classList.add("disabledtext"); 
															...
															do {
																...
																switch (parseInt(words[counter++])) {
																  ...
																  case ContainerTypes.FrameLabel:
																	...
																	imageframes.tail.value.label.addToTail(new FrameLabel(templabel.id, imageframes.tail.value, imageframes.tail.value.thisdiv, templabel.x, templabel.y, templabel.currenttext));
																	for (var i = 0; i < relevantdata_framelabel.length; i++) {
