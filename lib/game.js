			...
			show: function(opt) {
					if (opt.items["changevolume"]) {
						...
						volumeinput.addEventListener('mousedown', e => {
							var allpaused = function(somearrayofsounds) {
								var result = true;
								var origlength = somearrayofsounds.length;
								for (var i = origlength - 1; i >= 0; i--){
									if (!somearrayofsounds[i].paused)
										result = false;
								}
								return result;
							}
							if (audio_loop.paused && allpaused(audio_oneshot_collection)) {
