...
var relevantdata_soundboard = ["id", "owner", "timestamp", "tablecounter", "thistitletext", "labels", "filepaths", "pitches", "rates", "soundtypes"];
...
var relevantdata_soundboardentry_types = {"labels":"string", "filepaths":"string", "pitches":"number", "rates":"number", "soundtypes":"number"};
var imageframes = {};
var tokenframes = {};
var dieframes = {};
var publicdieframes = {};
...
$(function(){
	...
	iosocket.on('queueTTS', ( somemessage, somepitch, somerate ) => {
	...
	$.contextMenu({
				...
				if (topframe.what === ContainerTypes.FrameContainer || topframe.what === ContainerTypes.TokenContainer || topframe.what === ContainerTypes.Marker || topframe.what === ContainerTypes.FrameLabel || topframe.what === ContainerTypes.CanvasFrame || topframe.what === ContainerTypes.LotteryFrame || topframe.what === ContainerTypes.PublicDieFrame || topframe.what === ContainerTypes.Soundboard) {
					...
					thismenu.items["pushthis"] = {"name": "Push This", "callback": function(itemKey, opt){
													...
													if (actualframe.what === ContainerTypes.FrameContainer) {
														...
														if (new Blob([actualframe.content]).size > 8500000) {
															...
															if (e.dataTransfer.files[0].type.substring(0,6) == "image/") {
																...
																reader.onload = function () {
																	var applyuploadedimage = (function (sometopframe, somereaderresult){
																					// > 800KB for standard maxHttpBufferSize: 1e6
																					// > 8MB for maxHttpBufferSize: 1e7 in socket.io config in file 'index.js'
																					console.log(new Blob([somereaderresult]).size);
																					if (new Blob([somereaderresult]).size > 8500000) {
		...
		if (player === 0){ // && !topframe) {
				...
				thismenu.items.gameoptionsubmenu.items["addoption"] = {"name": "Add Option", "callback": function(itemKey, opt){
												var newoption = prompt("Current settings: " + gameoptions.join(' ') + "; supported yahtzee, notes, d<deckid>dragtotop, d<deckid>randomizeangles, d<deckid>cardturnangleinc<intValue>, fontDecorative, ...", "");
												if (newoption) {
				...
				thismenu.items["pushall"] = {"name": "Push All", "callback": function(itemKey, opt){
												...
												for (var canvasframe in canvasframes) {
													...
													// > 800KB for standard maxHttpBufferSize: 1e6
													// > 8MB for maxHttpBufferSize: 1e7 in socket.io config in file 'index.js'
													if (new Blob([canvasframes[canvasframe].content]).size > 8500000) {
														alert('This will possibly cause a connection error for canvas content size being too large.');
													}
													for (var j = 0; j < relevantdata_canvasframe.length; j++) {
...
function loadfromxml(somexml) {
	...
	for (var i = 0; i < somesoundboards.length; i++) {
		...
		for (var j = 0; j < somesoundboards[i].childNodes.length; j++) {
			if (somesoundboards[i].childNodes[j].nodeName == "#text" || somesoundboards[i].childNodes[j].nodeName == "labels" || somesoundboards[i].childNodes[j].nodeName == "filepaths" || somesoundboards[i].childNodes[j].nodeName == "pitches" || somesoundboards[i].childNodes[j].nodeName == "rates" || somesoundboards[i].childNodes[j].nodeName == "soundtypes") continue;
