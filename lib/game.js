...
$(function(){
	...
	iosocket.on('playsound', ( somesound, looping ) => {
		...
		if (looping) {
			...
			fadeAudio = setInterval(function () {
					// Only fade if past the fade out point or not at zero already
					if ((audio.currentTime >= audio.duration - fadeoutinterval) && (audio.volume > 0.0)) {
						audio.volume = Math.max(0.0, audio.volume - 0.1);
					}
					if ((audio.currentTime <= fadeoutinterval) && (audio.volume < basevolume)) {
						audio.volume = Math.min(basevolume, audio.volume + 0.1);
					}
				}, 200);
		}
		audio.play();
	...
	gamediv.addEventListener('wheel', function(e){
		...
		if (akeydown) {
			if (e.deltaY > 0) {
				basevolume = Math.max(0.0, basevolume - 0.025);
			}
			if (e.deltaY < 0) {
				basevolume = Math.min(1.0, basevolume + 0.025);
			}
...
function adjustzIndices() {
	// note: make this close the possible gaps
