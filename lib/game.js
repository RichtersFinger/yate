...
var editingnotes = false, changednotes = false;
var notesdiv_textdisplaydiv, notesdiv_textinputdiv, notesdiv_textinput;
...
$(function(){
	...
	notesdiv_textdisplaydiv.addEventListener('dblclick', e => {
		...
		if (!currentnotes[player]) 
			currentnotes[player] = "";
		
		notesdiv_textinput.value = currentnotes[player].replace(/<br>/g, '\n');
		changednotes = false;
	...
	document.getElementById("notesdiv_bapply").addEventListener('click', e => {
		...
		currentnotes[player] = notesdiv_textinput.value.replace(/\r?\n/g, '<br>');
		iosocket.emit('submitplayernotes', player, currentnotes[player]);
		notesdiv_textinput.focus();
	});
	document.getElementById("notesdiv_bclose").addEventListener('click', e => {
		...
		if (r) {
			...
			notesdiv_textdisplaydiv.innerHTML = currentnotes[player];
		}
	});
	// open/close notes paper; gameoption; drop-down select for GM; save load from file; disable apply button on server-respond
	...
	iosocket.on('updateplayernotes', ( someplayer, somenotes ) => {
		if (someplayer === player || player === 0) {
			...
			if (editingnotes && (player === someplayer || (currentnoteedit === someplayer && player === 0))) {
				console.log(somenotes);
	...
	iosocket.on('letpublicdieanimate', ( someid ) => {
		...
		if (publicdieframes[someid]) {
			var timer = setInterval(function () {
				...
				publicdieframes[someid].value = Math.floor(1 + publicdieframes[someid].maxvalue * random());
				...
				if (topframe.what === ContainerTypes.FrameContainer || topframe.what === ContainerTypes.TokenContainer || topframe.what === ContainerTypes.Marker || topframe.what === ContainerTypes.FrameLabel || topframe.what === ContainerTypes.CanvasFrame || topframe.what === ContainerTypes.LotteryFrame || topframe.what === ContainerTypes.PublicDieFrame) {
					...
					if (topframe.what === ContainerTypes.PublicDieFrame) {
						...
						if (topframe.animated)
							thismenu.items.framedetails.items["setabletoggle"] = {"name": "Animation: On", "callback": function(itemKey, opt){
																	if (topframe.animated) {
							...
							thismenu.items.framedetails.items["setabletoggle"] = {"name": "Animation: Off", "callback": function(itemKey, opt){
																	if (topframe.animated) {
																		topframe.animated = false;
																		opt.items.framedetails.items[itemKey].$node[0].innerHTML = "<span>Animation: Off</span>";
																	} else {
