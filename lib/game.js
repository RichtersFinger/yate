...
const ContainerTypes = {"FrameContainer":1, "TokenContainer":2, "Die":3, "Marker":4, "FrameLabel":5, "Card":6, "CanvasFrame":7, "LotteryFrame":8, "PublicDieFrame":9, "Soundboard":10};
...
var currentnotes = {};
var audio_loop = new Audio(), audio_oneshot = new Audio(), fadeAudio, basevolume = 0.25, akeydown = false;
...
var relevantdata_soundboard = ["id", "owner", "timestamp", "tablecounter", "thistitletext", "labels", "filepaths", "soundtypes"];
var relevantdata_soundboard_types = {"id":"number", "owner":"numberarray", "timestamp":"number", "tablecounter":"number", "thistitletext":"string", "labels":"stringarray", "filepaths":"stringarray", "soundtypes":"stringarray"};
var relevantdata_soundboardentry_types = {"labels":"string", "filepaths":"string", "soundtypes":"number"};
var imageframes = {};
var tokenframes = {};
var dieframes = {};
var publicdieframes = {};
	...
	iosocket.on('loginsuccess', ( playerid, playername ) => {
		...
		for (var soundboard in soundboards) {
			if (soundboards[soundboard].owner.includes(player)) {
				soundboards[soundboard].thisdiv.style.opacity = 1.0;
			} else {
				if (player === 0) {
					soundboards[soundboard].thisdiv.style.opacity = 1.0;
				}
	...
	$.contextMenu({
			...
			show: function(opt) {
					if (opt.items["changevolume"]) {
						...
						volumeinput.addEventListener('mousedown', e => {
							if (audio_loop.paused && audio_oneshot.paused) {
					...
					if (opt.items.sound) {
						if (opt.items.sound.items["changevolume"]) {
							...
							volumeinput.addEventListener('mousedown', e => {
								if (audio_loop.paused && audio_oneshot.paused) {
									audio_demo = new Audio("sound/_server_/noise.mp3");
									audio_demo.pause();
									audio_demo.volume = basevolume;
									audio_demo.loop = true;
									audio_demo.play();
		...
		build: function($trigger, e) {
			...
			if (player === 0 && topframe) {
				...
				if (topframe.what === ContainerTypes.FrameContainer || topframe.what === ContainerTypes.TokenContainer || topframe.what === ContainerTypes.Marker || topframe.what === ContainerTypes.FrameLabel || topframe.what === ContainerTypes.CanvasFrame || topframe.what === ContainerTypes.LotteryFrame || topframe.what === ContainerTypes.PublicDieFrame || topframe.what === ContainerTypes.Soundboard) {
					if (topframe.what === ContainerTypes.TokenContainer)
					...
					if (topframe.what === ContainerTypes.FrameContainer || topframe.what === ContainerTypes.TokenContainer || topframe.what === ContainerTypes.CanvasFrame || topframe.what === ContainerTypes.LotteryFrame || topframe.what === ContainerTypes.PublicDieFrame || topframe.what === ContainerTypes.Soundboard) {
					...
					thismenu.items["pushthis"] = {"name": "Push This", "callback": function(itemKey, opt){
													...
													//		if (relevantdata_soundboard[j] === "labels" || relevantdata_soundboard[j] === "filepaths" || relevantdata_soundboard[j] === "soundtypes") continue;
															tempframe[relevantdata_soundboard[j]] = actualframe[relevantdata_soundboard[j]];
														}
													/*	var j = 0;
														tempframe.labels = [];
														tempframe.filepaths = [];
														tempframe.soundtypes = [];
														for (var currententry in actualframe["labels"]) {
															tempframe.labels[j] = actualframe["labels"][currententry];
															tempframe.filepaths[j] = actualframe["filepaths"][currententry];
															tempframe.soundtypes[j] = actualframe["soundtypes"][currententry];
															j++;
														}*/
		...
		if (player === 0){ // && !topframe) {
				...
				thismenu.items["pushall"] = {"name": "Push All", "callback": function(itemKey, opt){
											...
											/*		var j = 0;
													...
													tempframe.filepaths = [];
													tempframe.soundtypes = [];
													for (var currententry in soundboards[soundboard]["labels"]) {
														tempframe.labels[j] = soundboards[soundboard]["labels"][currententry];
														tempframe.filepaths[j] = soundboards[soundboard]["filepaths"][currententry];
														tempframe.soundtypes[j] = soundboards[soundboard]["soundtypes"][currententry];
														j++;
													}*/
													iosocket.emit('pushsoundboard', tempframe);
												}
...
function loadfromxml(somexml) {
	...
	for (var i = 0; i < somesoundboards.length; i++) {
		...
		for (var j = 0; j < somesoundboards[i].childNodes.length; j++) {
			if (somesoundboards[i].childNodes[j].nodeName == "#text" || somesoundboards[i].childNodes[j].nodeName == "labels" || somesoundboards[i].childNodes[j].nodeName == "filepaths" || somesoundboards[i].childNodes[j].nodeName == "soundtypes") continue;
