...
$(function(){
	...
	iosocket.on('updatecard', ( somecard ) => {
		...
		if (correctcard) {
			...
			console.log(correctcard.onhand, !correctcard.faceup, correctcard.viewingrights.includes(player));
		...
		build: function($trigger, e) {
			...
			if (player === 0) {
				thismenu.items["createtestcard"] = {"name": "New Test Card", "callback": function(itemKey, opt){
												...
												decks.head.value.addToTail(new Card(1, idcounter++, decks.head.value, gamediv, InitialMouseDownX, InitialMouseDownY, 672/3, 936/3));
											}
							   };
				thismenu.items["createtestcard2"] = {"name": "New Test Card 2", "callback": function(itemKey, opt){
												...
												decks.tail.value.addToTail(new Card(2, idcounter++, decks.head.value, gamediv, InitialMouseDownX, InitialMouseDownY, 672/3, 936/3, "img/_server_/placeholdercard.jpg", "img/_server_/placeholdercardb.jpg"));
											}
							   };
			...
			if (player === 0 && topframe) {
				...
				if (topframe.what === ContainerTypes.FrameContainer || topframe.what === ContainerTypes.TokenContainer || topframe.what === ContainerTypes.Marker || topframe.what === ContainerTypes.FrameLabel) {
					...
					thismenu.items.cardsinglesubmenu.items["drawthis"] = {"name": "Draw", "callback": function(itemKey, opt){
													currentcardlist = new LinkedList();
													currentcardlist.addToTail(topframe);
													drawfunction();
												}
								   };
					thismenu.items.cardstacksubmenu.items["drawthisstack"] = {"name": "Draw", "callback": function(itemKey, opt){
													currentcardlist = multiselectlist;
													drawfunction();
												}
								   };	
					thismenu.items.carddecksubmenu.items["drawthisdeck"] = {"name": "Draw", "callback": function(itemKey, opt){
													var current = decks.head;
													for (var i = 0; i < decks.length; i++) {
														if (topframe.deckid === current.value.head.value.deckid) {
															currentcardlist = current.value;
					...
					thismenu.items.cardsinglesubmenu.items["discardthis"] = {"name": "Discard", "callback": function(itemKey, opt){
													currentcardlist = new LinkedList();
													currentcardlist.addToTail(topframe);
													discardfunction();
												}
								   };
					thismenu.items.cardstacksubmenu.items["discardthisstack"] = {"name": "Discard", "callback": function(itemKey, opt){
													currentcardlist = multiselectlist;
													discardfunction();
												}
								   };	
					thismenu.items.carddecksubmenu.items["discardthisdeck"] = {"name": "Discard", "callback": function(itemKey, opt){
													var current = decks.head;
													for (var i = 0; i < decks.length; i++) {
														if (topframe.deckid === current.value.head.value.deckid) {
															currentcardlist = current.value;
					...
					*/
					...
					thismenu.items.cardstacksubmenu.items["mixs"] = {"name": "Mix", "callback": function(itemKey, opt){
																var currentdate = new Date();
																var currenttime = currentdate.getTime();
																var current = multiselectlist.head;
																for (var i = 0; i < multiselectlist.length; i++) {
					...
					thismenu.items.carddecksubmenu.items["mixd"] = {"name": "Mix", "callback": function(itemKey, opt){
																var currentdate = new Date();
																var currenttime = currentdate.getTime();
																var current = topframe.deck.head;
																for (var i = 0; i < topframe.deck.length; i++) {
					...
					thismenu.items["fdiscardall"] = {"name": "Force Discard", "callback": function(itemKey, opt){
