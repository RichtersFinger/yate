...
class Card {
	constructor(somedeckid, somecardid, somedeck, parentdiv, x0, y0, w0, h0, frontfilename, backfilename) {
				...
				if (multiselected) {
					...
					for (var i = 0; i < multiselectlist.length; i++) {
						if (current.value.owner.includes(player)) {
							current.value.angle = Math.round(current.value.angle/90.0)*90;
							if (e.deltaY > 0.0) {
					...
					this.angle = Math.round(this.angle/90.0)*90;
					if (e.deltaY > 0.0) {
		...
		if (this.width < this.height) {
			this.thishandicon.style.width = Math.min(this.displaywidth, handiconscale*this.width)+"px";
			this.thishandicon.style.height = Math.min(this.displaywidth, handiconscale*this.width)+"px";
		} else {
			this.thishandicon.style.width = Math.min(this.displayheight, handiconscale*this.height)+"px";
			this.thishandicon.style.height = Math.min(this.displayheight, handiconscale*this.height)+"px";
		}
	}
	draw() {
