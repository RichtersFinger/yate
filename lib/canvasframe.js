...
class CanvasFrameContainer {
	...
	//	this.controlsredobutton.style.border = "2px solid #FFFFFF";
		...
		this.brushthicknessinput.max = "50";
		...
		this.brushcolorinput.value = "#000000";
		var changebrushsize = (function (canvasreference){
						return function(){	
							canvasreference.fabriccanvas.freeDrawingBrush.color = canvasreference.brushcolorinput.value;
						};
				}(this));
		this.brushcolorinput.onchange = changebrushsize;
		...
		this.controlsdiv.appendChild(this.brushcolorinput);
		this.controlsdiv.appendChild(document.createElement('br'));
		this.controlsdiv.appendChild(document.createElement('br'));
		...
		this.fabriccanvas.freeDrawingBrush.color = this.brushcolorinput.value;
		...
		var streamupdatefunction = (function (canvasreference){
						return function(){	
							if (canvasreference.owner.includes(player)) {
								...
								if (canvasreference.streamcontent) {
									var currentdate = new Date();
									canvasreference.timestamp = currentdate.getTime() - clienttimereference + servertimereference;//canvasreference.changinghistory = true;
									console.log('ad');
									iosocket.emit('requpdatecanvascontent', canvasreference.id, canvasreference.content, canvasreference.timestamp);
								}
								if (!canvasreference.changinghistory) {
									...
									var olength = canvasreference.history.length - 10;
