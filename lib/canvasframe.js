...
class CanvasFrameContainer {
	constructor(someid, parentdiv, x0, y0, w0, h0) {
		...
		var applyfunction = (function (canvasreference){
						return function(){	
							...
							canvasreference.thisdiv.style.zIndex = this.zIndex;
						};
				}(this));
	...
	//	this.controlsredobutton.style.border = "2px solid #FFFFFF";
		...
		this.brushcolorinput2.value = "#eb1717";
		...
		this.brushcolorinput3.value = "#4287f5";
		...
		this.brushcolorinput4.value = "#42f55a";
		...
		this.brushcolorinput5.value = "#f5ef42";
		...
		this.brushcolorinput6.value = "#ffffff";
						...
						return function(){
							...
							if (newcontent) {
								...
								if (canvasreference.streamcontent) {
									var currentdate = new Date();
									canvasreference.timestamp = currentdate.getTime() - clienttimereference + servertimereference;
									iosocket.emit('requpdatecanvascontent', canvasreference.id, newcontent, canvasreference.timestamp);
								}
							}
						};
				}(this));
	...
	loadproperties2(somecanvas) {
		var canvasproperties = Object.keys(somecanvas);
		for (var i = 0; i < canvasproperties.length; i++) {
			this[canvasproperties[i]] = somecanvas[canvasproperties[i]];
		}
