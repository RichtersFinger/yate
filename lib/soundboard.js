...
class Soundboard {
	constructor(someid, parentdiv, x0, y0, title) {
		...
		this.thisdiv.addEventListener('mousedown', e => {
			...
			if (e.target.nodeName != "BUTTON" && e.target.nodeName != "INPUT") { // && e.target.parentNode != this.thistitle
	...
	addnewentry(sometype, somelabel, somefilename, somepitch, somerate, somevolumemultiplier) {
		...
		if (player === 0) {
				...
				var removefunction = (function (sbreference){
								return function(){
									...
									delete sbreference.volumemultiplier[thisid];
									this.parentNode.parentNode.parentNode.removeChild(this.parentNode.parentNode);
									addtounpushedlist(sbreference);
								};
						}(this));
				...
				var volumemultiplierinput = document.createElement('input');
				volumemultiplierinput.setAttribute("type", "range");
				volumemultiplierinput.setAttribute("min", 0.0);
				volumemultiplierinput.setAttribute("max", 1.0);
				volumemultiplierinput.setAttribute("step", 0.05);
				if (somevolumemultiplier)
					volumemultiplierinput.setAttribute("value", somevolumemultiplier);
				else
					volumemultiplierinput.setAttribute("value", 0.5);
				volumemultiplierinput.style.width = "75px";
				volumemultiplierinput.id = "sbvolinputid_" + this.id + "." + this.tablecounter;

				var changedvolumefunction = (function (sbreference){
								return function(){
									var thisid = this.id.substring(this.id.indexOf(".") + 1);
									var currentdate = new Date();
									sbreference.timestamp = currentdate.getTime();
									sbreference.volumemultiplier[thisid] = volumemultiplierinput.value;
									iosocket.emit('changesoundboardsoundvolume', sbreference.id, thisid, volumemultiplierinput.value, sbreference.timestamp);
								};
						}(this));
				volumemultiplierinput.oninput = changedvolumefunction;
				var volumemultiplierinputcell = document.createElement('th');
				volumemultiplierinputcell.style.textAlign = "left";
				volumemultiplierinputcell.appendChild(volumemultiplierinput);
				somerow.appendChild(volumemultiplierinputcell);
		}
		...
		this.volumemultiplier[this.tablecounter] = somevolumemultiplier;
		if (player === 0) this.removelast();
		this.thistable.appendChild(somerow);
		if (player === 0) this.addrowtoadd();
		this.tablecounter++;
	}
	...
	opencreationdialog() {
		...
		var sbapplyfunction = (function (sbreference){
						return function(){
							...
							loadingsound = false;
							var thislabeltext = "", thisfilepath = "", thispitch = "", thisrate = "", thistype = "", thisvolumemultiplier = "";
							if (document.getElementById("soundboardaddtablink_bFile").classList.contains("active")) {
								...
								thisvolumemultiplier = document.getElementById("soundboardadddiv_File_volumeinput").value;
								thispitch = 1.0;
								thisrate = 1.0;
							}
							if (document.getElementById("soundboardaddtablink_bTTS").classList.contains("active")) {
								...
								thisvolumemultiplier = document.getElementById("soundboardadddiv_TTS_volumeinput").value;
								thisrate = document.getElementById("soundboardadddiv_TTS_rateinput").value;
							}
							sbreference.addnewentry(thistype, thislabeltext, thisfilepath, thispitch, thisrate, thisvolumemultiplier);
							addtounpushedlist(sbreference);
						};
				}(this));
		...
		document.getElementById("soundboardadddiv_File_btest").onclick = function(){
			...
			audio_oneshot.pause();
			if (basevolume === 0.0)
				audio_oneshot.volume = 0.5 * document.getElementById("soundboardadddiv_File_volumeinput").value;
			else
				audio_oneshot.volume = basevolume * document.getElementById("soundboardadddiv_File_volumeinput").value;
			audio_oneshot.play();
		};
		document.getElementById("soundboardadddiv_TTS_btest").onclick = function(){
			if (ttsvoice && document.getElementById("soundboardadddiv_TTS_textinput").value != "") {
				...
				if (basevolume === 0.0)
					msg.volume = 0.5 * document.getElementById("soundboardadddiv_TTS_volumeinput").value;
				else
					msg.volume = basevolume * document.getElementById("soundboardadddiv_TTS_volumeinput").value;
				msg.voice = ttsvoice;
	...
	loadproperties2(someframe) {
		...
		if (Object.keys(someframe["labels"]).length > 0) {
			...
			for (var entry in someframe["labels"]) {
				...
				if (!someframe["volumemultiplier"][entry]) someframe["volumemultiplier"][entry] = 0.5;
				this.addnewentry(someframe["soundtypes"][entry], someframe["labels"][entry], someframe["filepaths"][entry], someframe["pitches"][entry], someframe["rates"][entry], someframe["volumemultiplier"][entry]);
			}
