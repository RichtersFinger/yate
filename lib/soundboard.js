...
class Soundboard {
	...
	addnewentry(sometype, somelabel, somefilename) {
	...
	opencreationdialog() {
		...
		var sbapplyfunction = (function (sbreference){
						return function(){
							...
							loadingsound = false;
							var thislabeltext = "", thisfilepath = "", thistype = "";
							...
							sbreference.addnewentry(thistype, thislabeltext, thisfilepath);
						};
				}(this));
	...
	loadproperties2(someframe) {
		...
		if (Object.keys(someframe["labels"]).length > 0) {
			...
			for (var entry in someframe["labels"]) {
				...
				this.addnewentry(someframe["soundtypes"][entry], someframe["labels"][entry], someframe["filepaths"][entry]);
			}
